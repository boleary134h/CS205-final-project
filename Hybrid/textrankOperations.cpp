...
{
	#pragma omp parallel for shared(similarityMatrix,n_dim) private (i,j) // use collapse(2)
...
void TextRanker::pageRank()
{
	...
	#pragma omp parallel for shared(transition,n_dim) private(i,j)
	...
	#pragma omp parallel for shared(transition,n_dim,rowsum) private(i,j)
	double rowsum = 0.0;
	for (int i = 0; i < n_dim ; i++) {
		rowsum = 0.0;
		for (int j = 0; j < n_dim; j++) {
	...
	#pragma omp parallel for shared(transition,n_dim,damping) private(i,j)
	for (int i = 0; i < n_dim; i++) {
		for (int j = 0; j < n_dim; j++) {
	...
	*/
	...
	#pragma omp parallel for shared(cs,n_dim) private(i)
	for (int i = 0; i < n_dim; i++) {
	...
	while (iteration < max_iterations) {
		...
		#pragma omp parallel for shared(temp_centrality,n_dim) private(i)
		...
		#pragma omp parallel for shared(temp_centrality,cs,error,n_dim) private(i)
		double error = 0.0;
		for (int i = 0; i < n_dim; i++) {
		...
		// transfer centrality
		#pragma omp parallel for shared(temp_centrality,cs,n_dim) private(i)
		for (int i = 0; i < n_dim; i++) {
